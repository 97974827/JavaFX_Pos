...
public class MifareClassic
{
	...
	public byte[] readBinaryBlock(byte blockNum, byte length) throws Exception	
	{
		...
		System.arraycopy(apdu.getReceiveData(), 0, tmpArray, 0, 16);
			
		return tmpArray;
	}
	
	public boolean updateBinaryBlock(byte blockNum, byte[] data, byte length) throws Exception
	{
		Apdu apdu;
...
//			throw new Exception("Update failed");
	...
	public String readSerialNumber() throws Exception	
	{
		String serial1 = "";
		String serial2 = "";
		String serial3 = "";
		...
		apdu = new Apdu();
		apdu.setCommand(new byte[] { (byte)0xFF, (byte)0xCA, 0x00, 0x00, 0x00 });
		apdu.setSendData(new byte[0]);
		...
		serial = apdu.getReceiveData();
		for (int i = 0; i<serial.length; i++) {
			if (i == 0) {
				serial4 = String.format("%02X", serial[i]);
			} else if (i == 1) {
				serial3 = String.format("%02X", serial[i]);
			} else if (i == 2) {
				serial2 = String.format("%02X", serial[i]);
			} else if (i == 3) {
				serial1 = String.format("%02X", serial[i]);
			}					
		}
		
		final_serial = serial1 + serial2 + serial3 + serial4;
			
		return final_serial;
	}
	
	public void buzzer_control() throws Exception	
	{
		Apdu apdu;		
...
//		duration[0] = (byte) 0x10;
		
		apdu.setCommand(new byte[] { (byte)0xE0, (byte)0x00, (byte)0x00, (byte)0x28, (byte)0x01});
		apdu.setSendData(new byte[0]);
		apdu.setSendData(new byte[] {(byte)0x10});
